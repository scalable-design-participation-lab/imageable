...
def test_valid_camera_parameters():
    camera_params = CameraParameters(
        longitude = -73.983138, latitude = 40.763565 ,fov=90, heading=180, pitch=0, width=320, height=320
    )
...
def test_invalid_width():
    with pytest.raises(ValueError, match="width cannot be greater than 640"):
        CameraParameters(longitude = -73.983138, latitude = 40.763565, fov=90, heading=0, pitch=0, width=1000, height=530)
...
def test_invalid_height():
    with pytest.raises(ValueError, match="height cannot be greater than 640"):
        CameraParameters(longitude = -73.983138, latitude = 40.763565, fov=90, heading=0, pitch=0, width=640, height=1000)
...
def test_invalid_fov():
    with pytest.raises(ValueError, match="FOV should be between 10 and 120"):
        CameraParameters(longitude = -73.983138, latitude = 40.763565, fov=200, heading=0, pitch=0)


# Raises conversion from CameraParameters to dictionary
def test_to_dict_conversion():
    params = CameraParameters(longitude = -73.983138, latitude = 40.763565, fov=90, heading=0, pitch=0, width=640, height=640)
