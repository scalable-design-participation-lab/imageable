...
class HeightEstimationInput:
    ...
    vanishing_points: Optional[np.ndarray] = None  # Vanishing points (3, 2) or None
    ground_truth: Optional[np.ndarray] = None  # Ground truth height map (H, W) or None
...
class CameraParameters:
    ...
    @classmethod
    def from_fov(cls, fov_degrees: float, image_width: int = 640, image_height: int = 640):
...
class HeightCalculator:
    ...
    def calculate_heights(
        ...
        try:
            ...
            print(f"Error in height calculation: {str(e)}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _get_vanishing_points(
        self,
        provided_vps: Optional[np.ndarray],
        camera: CameraParameters,
        ...
        if use_pitch_only:
            if pitch is None:
                raise ValueError("Pitch angle required when use_pitch_only=True")
    ...
    def _filter_lines_in_buildings(
        ...
        for (a, b), s in zip(lines, line_scores):
            ...
            if not self.line_classifier.check_if_line_lies_in_building_area(
                segmentation, a, b, self.config
            ):
                continue
    ...
    def _extend_vertical_lines(
        ...
        for line in vertical_lines:
            ...
            extd_a, extd_b = self.line_refiner.extend_lines(
                line[0], line[1], segmentation, self.config
            )
...
class ImageableHeightEstimator:
    ...
    def process(
        ...
        segmentation: np.ndarray,
        lines: np.ndarray,
        line_scores: Optional[np.ndarray] = None,
