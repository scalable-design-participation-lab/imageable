...
def nms_j(heatmap, delta=1):
    ...
    for x, y in argsort2d(heatmap):
        for dx, dy in zip(DX, DY, strict=False):
            xp, yp = x + dx, y + dy
...
def vectorized_wireframe_2d_metric(vert_pred, dpth_pred, edge_pred, vert_gt, dpth_gt, edge_gt, threshold):
    # staging 1: matching
    nd = len(vert_pred)
    sorted_confidence = np.argsort(-vert_pred[:, -1])
    vert_pred = vert_pred[sorted_confidence, :-1]
    dpth_pred = dpth_pred[sorted_confidence]
    d = np.sqrt(np.sum(vert_pred**2, 1)[:, None] + np.sum(vert_gt**2, 1)[None, :] - 2 * vert_pred @ vert_gt.T)
    ...
    for i, (v0, v1, score) in enumerate(sorted(edge_pred, key=-edge_pred[2])):
        ...
        if frozenset([choice[v0], choice[v1]]) in edgeset:
            ...
            fp[i] = length
    total_length = LA.norm(vert_gt[edge_gt[:, 0]] - vert_gt[edge_gt[:, 1]], axis=1).sum()
    return ap(tp / total_length, fp / total_length), (loss_SIL, loss_L1, loss_L2)


def vectorized_wireframe_3d_metric(vert_pred, dpth_pred, edge_pred, vert_gt, dpth_gt, edge_gt, threshold):
    # staging 1: matching
    nd = len(vert_pred)
    sorted_confidence = np.argsort(-vert_pred[:, -1])
