...
from imageable.models.materials.backbones.segformer_mit_encoder import (
...
class RMSNet(nn.Module):
    ...
    """
    def __init__(self, num_classes: int = 20, backbone: str = "mit_b2",
                 sync_bn: bool = False, freeze_bn: bool = False, model_path = None) -> None:
        super().__init__()
        ...
        norm_layer = (SynchronizedBatchNorm2d if (sync_bn and SynchronizedBatchNorm2d is not None)
                      else nn.BatchNorm2d)
    ...
    def freeze_bn(self) -> None:
        ...
        for m in self.modules():
            if isinstance(m, (nn.BatchNorm2d,)) or (SynchronizedBatchNorm2d is not None and isinstance(m, SynchronizedBatchNorm2d)):
                m.eval()
