...
class CameraParametersEstimator:
    ...
    def estimate_first_parameters(
        self,
        buffer_constant: float = 2.5e5)->CameraParameters:
        """
...
class CameraParametersRefiner:
    ...
    def adjust_parameters(
        ...
        camera_parameters = params_estimator.estimate_first_parameters(
            buffer_constant=polygon_buffer_constant
        )
        ...
        while(not view_obtained):
            ...
            if(image is not None):
                ...
                if((sky_ratio >= 0 and sky_ratio<=self.MIN_SKY_RATIO) and floor_ratio > self.MIN_FLOOR_RATIO):
                    ...
                    (sky_ratio >= 0 and sky_ratio <= self.MIN_SKY_RATIO)
                    and (floor_ratio >= 0 and floor_ratio <= self.MIN_FLOOR_RATIO)
                ):
                    #Increase the FOV
