...
class FootprintCalculator:
    ...
    def shape_length(footprint: Polygon) -> float:
        ...
        for i in range(len(poly_coords) - 1):
            ...
            dst = np.sqrt(
                (next_vertex[0] - current_vertex[0])**2 + 
                (next_vertex[1] - current_vertex[1])**2
            )
            shape_length += dst
        return shape_length
    ...
    def neighbor_count(
        ...
        """
        _, filtered_footprints = FootprintCalculator.get_footprints_within_radius(
            footprints,
            center_footprint,
            radius
        )
    ...
    def mean_distance_to_neighbors(
        ...
        distances = [
            np.sqrt(
                (nc.x - centroid_center.x)**2 + 
                (nc.y - centroid_center.y)**2
            )
    ...
    def nearest_neighbor_distance(
        ...
        for neighbor in neighbors:
            ...
            dst = np.sqrt(
                (neighbor_coords.x - polygon_coords.x)**2 + 
                (neighbor_coords.y - polygon_coords.y)**2
            )
    ...
    def nni(
        ...
        """
        ond = FootprintCalculator.nearest_neighbor_distance(
            footprints,
            center_building,
            radius
        )
...
def extract_footprint_properties(
    polygon: Polygon,
    all_footprints: Optional[List[Polygon]] = None,
    crs: int = 4326,
    neighbor_radius: float = 600
) -> Dict[str, Any]:
    """
    ...
    if all_footprints is not None:
        ...
        projected_center, neighbors = calc.get_footprints_within_radius(
            all_footprints,
            polygon,
            neighbor_radius
        )
        ...
        if n_count > 0:
            ...
            distances = [
                np.sqrt(
                    (nc.x - center_centroid.x)**2 + 
                    (nc.y - center_centroid.y)**2
                )
                for nc in neighbor_centroids
            ]
            ...
            properties['n_size_cv'] = (
                properties['n_size_std'] / mean_area if mean_area > 0 else 0.0
            )
            ...
            properties['nni'] = (
                observed_nnd / expected_nnd 
                if expected_nnd > 0 and expected_nnd != float("inf") 
                else float("inf")
            )
        else:
