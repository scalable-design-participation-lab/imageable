...
class MixVisionTransformer(nn.Module):
    def __init__(
        ...
        # patch embeds (SegFormer-style overlapping)
        ...
        # optional pretrained backbone weights
    ...
    def _load_pretrained_model(self, src: str | Path | dict[str, Any]) -> None:
        ...
        Load pretrained weights into the backbone. Accepts:
          - path to .pth/.pt
          - already loaded dict (state_dict or full checkpoint)
        Handles common formats:
          - flat state_dict
          - {'state_dict': ..., ...}
          - keys prefixed with 'backbone.' (will be stripped)
        """
        ...
        # state must be a dict of tensors
        if not isinstance(state, dict):
            raise RuntimeError("Unsupported checkpoint format for MixVisionTransformer")

        # strip 'backbone.' or similar prefixes if present
        cleaned = {}
        ...
        # You can print or log these if you want:
        # print(f"[MVT] loaded with {len(missing)} missing, {len(unexpected)} unexpected keys")
