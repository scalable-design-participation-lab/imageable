...
class LineRefiner:
    def extend_lines(self, pt1, pt2, segmt, config):
        ...
        # Boundary checks
        pt_up_end[0] = min(pt_up_end[0], rows - 2)
        pt_up_end[1] = min(pt_up_end[1], cols - 2)
        pt_down_end[0] = min(pt_down_end[0], rows - 2)
        pt_down_end[1] = min(pt_down_end[1], cols - 2)
        ...
        while flag:
            ...
            if pt_up_end[0] < 0 or pt_up_end[1] < 0 or pt_up_end[1] >= rows - 1:
                ...
                pt_up_end = pt_up_end + direction
                continue
            if segmt[int(pt_up_end[0] + 0.5)][int(pt_up_end[1] + 0.5)] == int(sky_label[0]) or segmt[
                int(pt_up_end[0] + 0.5)
            ][int(pt_up_end[1] + 0.5)] == int(sky_label[1]):
                flag = 0
                pt_up_end = pt_up_end + direction
        ...
        while flag:
            ...
            elif current_label == building_label:
                out_of_building = False

            if current_label in ground_label and not out_of_building:
                flag = 0
                pt_down_end = pt_down_end - direction
            elif current_label in ground_label:
                return [], []

        # Check edge thresholds
        if (
            pt_up_end[0] > cols - 1 - edge_thres[0]
            or pt_up_end[1] < edge_thres[0]
            or pt_up_end[1] > rows - edge_thres[0]
            or pt_down_end[0] < edge_thres[0]
            or pt_down_end[0] > cols - 1 - edge_thres[0]
            or pt_down_end[1] < edge_thres[0]
            or pt_down_end[1] > rows - 1 - edge_thres[0]
        ):
            return [], []
